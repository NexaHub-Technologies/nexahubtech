	...
	head: () => ({
		meta: [
			{ title: "About — NexaHubTech" },
			{
				name: "description",
				content:
					"NexaHubTech (formerly Codex Technologies) is a Nigeria-based skills-to-service company — a tech school and a software studio built on one idea: learning should be tied to real work.",
			},
		],
	}),
});
		...
		<>
			<section className="wrap page-head">
				<div className="max-w-[46rem]">
					...
					<h1 className="display rise rise-2 mt-6 text-[2.6rem] sm:text-[3.4rem]">
						...
						<span style={{ color: "var(--teal-deep)" }}>doing</span>.
					</h1>
					...
					<div className="grid md:grid-cols-2 gap-10">
						<div>
							...
							<h2 className="display mt-4 text-[1.9rem]">
								From Codex to NexaHubTech.
							</h2>
							<p className="lead mt-4">
								We started as Codex Technologies and grew into something with a
								clearer purpose: not just building software, and not just
								teaching it, but wiring the two together.
							</p>
							<p className="devnote mt-4">
								Add the rebrand story & founding year when you’re ready.
							</p>
						</div>
						<div>
			...
			<section className="wrap py-14">
				<div className="card reveal p-8 md:p-12 grid md:grid-cols-[auto_1fr] gap-8 items-start">
					<div
						className="w-20 h-20 rounded-3xl grid place-items-center shrink-0"
						style={{
							background:
						...
						aria-hidden="true"
					>
						<span className="display text-white text-3xl">M</span>
					</div>
					<div>
						<Eyebrow variant="plain">From the founder</Eyebrow>
						<p className="display text-[1.5rem] sm:text-[1.85rem] mt-3 max-w-[46rem] leading-[1.25]">
							...
							real problems. So we built a place where the teaching and the
							building grow together.”
						</p>
						<p className="lead mt-5 max-w-[44rem]">
							Mich (Michael) founded NexaHubTech to close the gap between
							learning to code and doing the work. He teaches, builds for
							clients, and reasons from first principles — equally at home with
							how a render engine works, an economics question, or a tricky data
							model.
						</p>
						<p className="devnote mt-5">
							Founder photo, full display name & a personal note — to add before
							launch.
						</p>
					</div>
				</div>
